...
session_start();
                ...
                for($i=1; $i<=$countID; $i++)
                  {
                     ...
                     echo "'{$emp}','{$countID}', '{$subtotal}', '{$lessvat}','{$addvat}','{$total}','{$cash}','{$date}','{$today} <br> ";
          ...
          mysqli_query($db,$query)or die (mysqli_error($db));
                    ...
                    $query11="SELECT * FROM employee WHERE FIRST_NAME = '".$emp."'";
        $result1 = mysqli_query($db, $query11) or die (mysqli_error($db));

        $row = mysqli_fetch_assoc($result1);
        $id = $row['EMPLOYEE_ID'];
                    $query111 = "INSERT INTO `transactions`
                               (`TRANS_ID`, `EMPLOYEE_ID`, `NUMOFITEMS`, `SUBTOTAL`, `LESSVAT`, `NETVAT`, `ADDVAT`, `GRANDTOTAL`, `CASH`, `DATE`, `TRANS_D_ID`)
                               VALUES (Null,'{$id}','{$countID}','{$subtotal}','{$lessvat}','{$netvat}','{$addvat}','{$total}','{$cash}','{$date}','{$today}')";
